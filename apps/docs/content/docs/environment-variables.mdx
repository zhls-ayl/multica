...
| DingTalk | `MULTICA_DINGTALK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key; empty disables DingTalk |
| WeCom | `MULTICA_WECOM_SECRET_KEY` | Base64-encoded 32-byte credential encryption key; empty disables WeCom |
| Telegram | `MULTICA_TELEGRAM_SECRET_KEY` | Base64-encoded 32-byte Bot token encryption key |
| ShareCRM | `MULTICA_SHARECRM_SECRET_KEY` | Base64-encoded 32-byte credential encryption key; empty disables ShareCRM |
...
For setup steps, see [GitHub integration](/github-integration#self-hosting-setup), [Lark bot](/lark-bot-integration#self-hosting-setup), [Slack bot](/slack-bot-integration#self-hosting-setup), [DingTalk bot](/dingtalk-bot-integration#self-host-setup), [Telegram bot](/telegram-bot-integration#self-hosting-setup), [ShareCRM bot](/sharecrm-bot-integration#self-host-setup), and [Chat integrations](/channels#self-hosting).
