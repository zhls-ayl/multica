...
// ShareCRMBindPage is the destination the bot's "link your account" prompt
...
// /api/sharecrm/binding/redeem; the backend returns 410 (invalid/expired),
// 409 (already bound to another user), 403 (not a workspace member) or 200 with
// the bound installation. Each maps to distinct copy via sharecrm_bind in
// common.json.
export function ShareCRMBindPage({ token }: { token: string | null }) {
  const { t } = useT("common");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await api.redeemShareCRMBindingToken(token);
        setState({
          kind: "done",
    ...
    <div className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center p-6">
      <Card className="w-full">
        <CardContent className="space-y-4">
          <h1 className="text-title font-semibold">{t(($) => $.sharecrm_bind.page_title)}</h1>
          {state.kind === "idle" || state.kind === "redeeming" ? (
            <p className="text-body text-muted-foreground">{t(($) => $.sharecrm_bind.redeeming)}</p>
          ) : state.kind === "needs-auth" ? (
            <>
              <p className="text-body text-muted-foreground">
                {t(($) => $.sharecrm_bind.needs_auth_description)}
              </p>
              <Button
                size="sm"
                onClick={() =>
                  navigation.push(
                    `/login?next=${encodeURIComponent(
                      `/sharecrm/bind?token=${encodeURIComponent(token ?? "")}`,
                    )}`,
                  )
                }
              >
                {t(($) => $.sharecrm_bind.sign_in)}
              </Button>
            </>
          ) : state.kind === "done" ? (
            <>
              <p className="text-body font-medium">{t(($) => $.sharecrm_bind.done_title)}</p>
              <p className="text-caption text-muted-foreground">
                {t(($) => $.sharecrm_bind.done_description)}
              </p>
            </>
          ) : (
            <>
              <p className="text-body font-medium">{t(($) => $.sharecrm_bind.error_title)}</p>
              <p className="text-caption text-muted-foreground">
                {(() => {
                  switch (state.reason) {
                    case "missing_token":
                      return t(($) => $.sharecrm_bind.error_missing_token);
                    case "expired":
                      return t(($) => $.sharecrm_bind.error_expired);
                    case "already_bound":
                      return t(($) => $.sharecrm_bind.error_already_bound);
                    case "not_member":
                      return t(($) => $.sharecrm_bind.error_not_member);
                    default:
                      return t(($) => $.sharecrm_bind.error_unknown);
                  }
                })()}
              </p>
              <p className="text-micro text-muted-foreground">
                {t(($) => $.sharecrm_bind.error_admin_hint)}
              </p>
            </>
          )}
        </CardContent>
