...
type fakeInstallQueries struct {
	...
	lockCalled    bool
	replaceCalled bool
	replaceParams db.DeleteShareCRMInstallationForReplacementParams
	// appIDTaken makes UpsertChannelInstallation report a unique-constraint
	// violation on the (channel_type, app_id) routing index.
	appIDTaken   bool
...
func (f *fakeInstallQueries) LockShareCRMInstallationOwner(_ context.Context, _ db.LockShareCRMInstallationOwnerParams) error {
	f.lockCalled = true
	return nil
}

func (f *fakeInstallQueries) GetShareCRMInstallationOwnerForUpdate(_ context.Context, _ db.GetShareCRMInstallationOwnerForUpdateParams) (db.GetShareCRMInstallationOwnerForUpdateRow, error) {
	if f.existing == nil {
		return db.GetShareCRMInstallationOwnerForUpdateRow{}, pgx.ErrNoRows
	}
	return db.GetShareCRMInstallationOwnerForUpdateRow{ID: f.existing.ID, AppID: f.existingAppID}, nil
}

func (f *fakeInstallQueries) DeleteShareCRMInstallationForReplacement(_ context.Context, arg db.DeleteShareCRMInstallationForReplacementParams) (pgtype.UUID, error) {
