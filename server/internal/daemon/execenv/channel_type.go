...
// discriminator), Feishu, WeCom, DingTalk and ShareCRM all persist via the
// shared AppendUserMessage path; Slack reads the live channel instead. It is the single
...
func SurfacePersistsTranscript(channelType string) bool {
	switch channelType {
	case "", ChannelTypeFeishu, ChannelTypeWecom, ChannelTypeDingtalk, ChannelTypeSharecrm:
		return true
	default:
		return false
	}
